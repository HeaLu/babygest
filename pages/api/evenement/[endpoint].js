...
export default async function handler(req, res) {
  ...
  switch (endpoint) {
    case "getlast":
      ...
      try {
        let data = await Evenement.findOne().sort({ date: -1 });
        if (!data) {
          data = new Evenement({ date: startOfDay(new Date()), user });
        }
    ...
    case "add":
      ...
      try {
        ...
        if (!body.message) return res.status(400).json({ message: "Message manquant" })
        const data = new Evenement({ date: new Date(body.date), message: body.message, user })
        await data.save()
        return res.json(data);
      } catch (err) {
        return res.status(500).json({ message: "Erreur inconnue" });
